...
class M6Replay( Plugin ):
	...
	def rafraichir( self ):
		...
		self.afficher( str( len( self.listeProgrammes.keys() ) ) + " émissions conservées." )
	...
	def listerFichiers( self, emission ):
		if( self.listeProgrammes.has_key( emission ) ):
			infosEpisodes = self.listeProgrammes[ emission ]
			for ( nom, date, lien, urlImage, descriptif ) in infosEpisodes:
...
class M6ReplayHandler( ContentHandler ):
	...
	def endElement( self, name ):
		if( name == "categorie" ):
			...
			self.listeVideos.append( [ self.nomEpisode, self.dateEpisode, self.nomVideo, self.image, self.resume ] )
			self.nomEpisodeConnu = False
			self.nomVideoConnu = False
