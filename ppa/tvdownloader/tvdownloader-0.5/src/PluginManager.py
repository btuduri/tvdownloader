...
class PluginManager( object ):
	...
	def __init__( self ):
		...
		for rep in repPlugins:
			...
			if( not os.path.isdir( rep ) ):
				print "Ouuupppss : le repertoire %s des plugins n'existe pas..." %( rep ) 
			...
			for fichier in files: 
				...
				if( fichier [ -3 : ] == ".py" and fichier.find( "__init__.py" ) == -1 ):
					try :
						...
						print "Erreur : impossible d'importer le fichier %s" %( fichier )
						continue
	...
	def activerPlugin( self, nomPlugin ):
		if( self.listePlugins.has_key( nomPlugin ) ):
			...
			print "PluginManager.activerPlugin : impossible d'activer le plugin %s" %( nomPlugin )
	...
	def desactiverPlugin( self, nomPlugin ):
		...
		if( self.listeInstances.has_key( nomPlugin ) ):
			...
			print "PluginManager.desactiverPlugin : impossible de desactiver le plugin %s" %( nomPlugin )
