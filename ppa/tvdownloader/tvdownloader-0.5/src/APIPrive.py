...
class APIPrive():
	...
	def setListeInstance(self, liste):
		print "DEPRECATED: APIPrive.setListeInstance: utilisez #ajouterPlugin"
		self.listePluginActif = {}
	...
	def pluginRafraichir(self, nomPlugin):
		try:
			...
			print "Erreur lors du rafraichissement du plugin",self.listePlugin[nomPlugin].nom+":",ex
			print_exc()
		...
		for instance in self.listePlugin.values():
			...
			if os.path.exists(instance.fichierConfiguration):
				try:
					...
					print "Erreur de chargement des préférences de",instance.nom
		...
		for instance in self.listePluginActif.values():
			if len(instance.pluginOptions) > 0:
				try:
					...
					print "Erreur de sauvegarde des préférences de",instance.nom
	...
	def reponseHttpToUTF8(self, reponse):
		...
		if reponse.getheader('Content-Encoding', '').find('gzip') >= 0:
			try:
				...
				print "APIPrive.reponseHttpToUTF8(): erreur de décompression"
				return ""
		else:
			data = reponse.read()
		...
		if encoding != None:
			try:
				...
				if match != None:
					...
					print "APIPrive.reponseHttpToUTF8(): encodage inconnu."
			except:
				print "APIPrive.reponseHttpToUTF8(): encodage inconnu."
		iters = []
