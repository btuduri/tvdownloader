...
class CanalPlus(Plugin):
	...
	listeEmissionsPattern = re.compile("<div class=\"col\">.*?<h3>([\w ]+)</h3>.*?<ul>(.+?)</ul>", re.IGNORECASE|re.DOTALL)
	listeEmissionsSubPattern = re.compile(" href=\"(http://www.canalplus.fr/pid\d+?.htm)\">(.+?)</", re.IGNORECASE|re.DOTALL)
	emissions_NOM = 1
	emissions_LIEN = 0
