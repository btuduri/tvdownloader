...
class Plugin(object):
	...
	def __init__(self, nom=None, url=None, frequence=7):
		...
		if nom == None:
			print "DEPRECATED: il faut préciser le nom du plugin au constructeur"
		else:
			self.nom = nom
		
		if url == None:
			print "DEPRECATED: il faut préciser l'url du site du plugin au constructeur"
		else:
			self.url = url
	...
	# @param ligne si vraie, l'affichage passe à la ligne après "text"
	# @return Rien
	def afficher(self, text, ligne=True):
		if ligne:
			print self.nom+":",text
		else:
			print self.nom+":",text,
	...
	def sauvegarderPreference(self):
		print "Plugin.sauvegarderPreference(): DEPRECATED: ne fait plus rien."
		return
		try:
	...
	def chargerPreference(self):
		print "Plugin.chargerPreference(): DEPRECATED: ne fait plus rien."
		return
	...
	def sauvegarderCache(self, objet):
		try:
			...
			print "Plugin.sauvegarderCache(): Erreur de sauvegarde"
