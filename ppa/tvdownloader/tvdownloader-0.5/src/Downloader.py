...
class Downloader( object ):
	...
	def lancerTelechargement( self, listeFichiers ):
		...
		for ( numero, fichier, nomFichierSortie ) in listeFichiers:
			...
			if( fichier[ :4 ] == "rtmp" ):
				...
				print "!!! Erreur !!! : Le protocole du fichier %s n'est pas geree" %( fichier )	
			# Si on a le lanceur de signal
			if self.signaux :
	...
	def telecharger( self, commande ):
		...
		if( not self.signaux and not isinstance( commande, str ) ):
			try:
				commande = str( commande.toUtf8() )
			except UnicodeDecodeError:
				commande = unicode( commande.toUtf8(), "utf-8" )
