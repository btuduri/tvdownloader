; Fichier NSIS pour TVDownloader
...
;--------------------------------
	...
	; Nom de l'executable
	OutFile "TVDownloader_0.7.2.exe"

	; Dossier d'installation par defaut
	InstallDir "$PROGRAMFILES\TVDownloader"

	; Recupere si possible le repertoire d'installation dans le registre (pour les mises a jour)
	InstallDirRegKey HKLM "Software\TVDownloader" "Install_Dir"
...
;--------------------------------
	...
	; Section qui comprend le coeur du programme
		...
		Section "TVDownloader" TVD
			...
			WriteRegStr HKLM "Software\TVDownloader" "Install_Dir" "$INSTDIR"
			...
			CreateDirectory "$SMPROGRAMS\TVDownloader"
			; On cree le raccourci du programme
			CreateShortCut "$SMPROGRAMS\TVDownloader\TVDownloader.lnk" "$INSTDIR\main.exe" "" "$INSTDIR\ico\TVDownloader.ico" 0
			; On cree le raccourci du desinstalleur
			CreateShortCut "$SMPROGRAMS\TVDownloader\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
			
			; Raccourci sur le bureau
			CreateShortCut "$DESKTOP\TVDownloader.lnk" "$INSTDIR\main.exe" "" "$INSTDIR\ico\TVDownloader.ico" 0
...
;--------------------------------
	...
	Section "Uninstall"
		...
		Delete "$DESKTOP\TVDownloader.lnk"
		Delete "$SMPROGRAMS\TVDownloader\Uninstall.lnk"
		Delete "$SMPROGRAMS\TVDownloader\TVDownloader.lnk"
		RMDir "$SMPROGRAMS\TVDownloader"

		; On supprime la clef registre
		DeleteRegKey /ifempty HKLM "Software\TVDownloader"
