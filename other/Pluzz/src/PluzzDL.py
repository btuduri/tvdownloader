...
class PluzzDL( object ):
	...
	def getNomFichier( self, extension ):
		...
		"""
		self.nomFichier = os.path.join( self.outDir, "%s_%s.%s" % ( self.codeProgramme, datetime.datetime.fromtimestamp( self.timeStamp ).strftime( "%Y-%m-%d_%H-%M" ), extension ) )
